...
document.addEventListener("DOMContentLoaded", async () => {
  console.log("🚀 DisasterOS Citizen Dashboard");
...
function loadUser() {
  ...
  if (storedUser && userName) {
    userName.textContent =
      storedUser.name || storedUser.username || storedUser.email || "Citizen";
  }
}
...
function updateLocationUI() {
  ...
  if (dashboardLocation.latitude === null) {
...
function renderPrediction() {
  ...
  if (riskProgress) {
    riskProgress.style.width = `${Math.min(100, Math.max(0, probability))}%`;
...
function renderWeather() {
  ...
  const temperatureValue = weatherData.temperature ?? weatherData.temp ?? "--";

  const humidityValue = weatherData.humidity ?? "--";

  const windValue =
    weatherData.windSpeed ?? weatherData.wind_speed ?? weatherData.wind ?? "--";
...
async function loadResources() {
  ...
  try {
    ...
    if (!response.success) {
      throw new Error(response.message || "Resources unavailable");
    }

    resourcesData = response.resources || response.data || {};
    ...
    setResourceCount(hospitalCount, "--");

    setResourceCount(fireCount, "--");

    setResourceCount(policeCount, "--");

    setResourceCount(shelterCount, "--");
  }
}
...
function renderResources() {
  ...
  const hospitals = resourcesData.hospitals || [];

  const fireStations = resourcesData.fireStations || [];

  const policeStations = resourcesData.policeStations || [];

  const shelters = resourcesData.shelters || [];
...
function renderAlerts(incidents) {
  ...
  if (alertCount) {
    alertCount.textContent = incidents.length;
  }

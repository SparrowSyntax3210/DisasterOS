...
/* ============================================================
   HIDE COMMAND CENTER LOADER
   ============================================================ */

function hideCommandCenterLoader() {
  const loader = document.getElementById("dataLoader");

  if (!loader) {
    console.warn("[COMMAND CENTER] Loader element not found.");
    return;
  }

  loader.classList.add("hidden");

  // Remove it completely after the fade-out animation.
  setTimeout(() => {
    loader.style.display = "none";
  }, 500);

  console.log("[COMMAND CENTER] Loading screen hidden.");
}
...
/* ============================================================
   COMMAND CENTER LOCATION SYSTEM
============================================================ */

function getCommandLocationState() {
  return (
...
function hideCommandLocationGate() {
  const gate = document.getElementById("commandLocationGate");
...
async function waitForCommandCenterAuth() {
  if (window.commandCenterAuthReady) {
    const user = await window.commandCenterAuthReady;
  ...
  /*
   * Fallback if auth promise wasn't created.
   */

  if (typeof protectDashboard === "function") {
    const user = await protectDashboard("command-center");
...
async function selectCommandCenterLocation(latitude, longitude, name) {
  ...
  console.log("[COMMAND CENTER] Location selected:", CommandCenter.location);
  sessionStorage.setItem(
    "commandCenterLocation",
    JSON.stringify(CommandCenter.location),
  );

  hideCommandLocationGate();
  try {
    initializeCommandCenterGIS();
  } catch (error) {
    console.error("[COMMAND CENTER] GIS initialization failed:", error);
  }

  try {
    await initializeCommandCenterDashboard();
  } catch (error) {
    console.error("[COMMAND CENTER] Dashboard initialization failed:", error);
  }

  focusCommandCenterMap(latitude, longitude, 12);
  ...
  try {
    ...
    console.warn("[COMMAND CENTER] Nearby resource loading failed:", error);
  }

  /*
   * Now load the operational dashboard.
   */

  await loadCompleteCommandCenterData();

  return CommandCenter.location;
}
...
async function searchCommandCenterLocation() {
  const input = document.getElementById("commandLocationInput");

  const status = document.getElementById("commandLocationStatus");
  ...
  if (!location) {
    if (status) {
      ...
      status.className = "command-location-status error";
    }
  ...
  try {
    if (status) {
      status.textContent = "Locating operational area...";

      status.className = "command-location-status";
    }
    ...
    if (latitude === null || longitude === null) {
      throw new Error("Location does not contain valid coordinates.");
    }

    const name = result.name || result.display_name || location;

    await selectCommandCenterLocation(latitude, longitude, name);

    if (status) {
      status.textContent = `Operational area: ${name}`;

      status.className = "command-location-status success";
    }
  } catch (error) {
    console.error("[COMMAND CENTER] Location search failed:", error);

    if (status) {
      status.textContent = error.message || "Location not found.";
...
function useCommandCenterLiveLocation() {
  const status = document.getElementById("commandLocationStatus");
  ...
  navigator.geolocation.getCurrentPosition(
    async (position) => {
      try {
        ...
        await selectCommandCenterLocation(
          latitude,
          longitude,
          "Current Location",
        );
        ...
        if (status) {
          status.textContent =
            error.message || "Unable to load current location.";
    ...
    (error) => {
      console.error("[COMMAND CENTER] Geolocation error:", error);

      if (status) {
        status.textContent = "Unable to detect your current location.";
...
function initializeCommandCenterLocationGate() {
  const searchButton = document.getElementById("commandLocationSearchBtn");

  const liveButton = document.getElementById("commandLiveLocationBtn");

  const input = document.getElementById("commandLocationInput");

  if (searchButton) {
    searchButton.addEventListener("click", searchCommandCenterLocation);
  }

  if (liveButton) {
    liveButton.addEventListener("click", useCommandCenterLiveLocation);
  }
  ...
  try {
    const saved = JSON.parse(
      sessionStorage.getItem("commandCenterLocation") || "null",
    );
    ...
    if (
      ...
      CommandCenter.location = {
        ...
        radius: Number(saved.radius) || 25000,
      };
    ...
    console.warn("[COMMAND CENTER] Saved location invalid.");
  }

  showCommandLocationGate();
...
window.selectCommandCenterLocation = selectCommandCenterLocation;

window.searchCommandCenterLocation = searchCommandCenterLocation;

window.useCommandCenterLiveLocation = useCommandCenterLiveLocation;

window.getCommandCenterLocation = () => CommandCenter.location;
...
/* ============================================================
   COMMAND CENTER MAP ICON
   ============================================================ */

function createCommandCenterIcon(type = "default", status = "") {
  const normalizedType = String(type || "default")
    .trim()
    .toLowerCase();
  ...
  const emoji = iconMap[normalizedType] || iconMap.default;

  let statusClass = "default";

  if (
    ["ACTIVE", "OPEN", "IN_PROGRESS", "DISPATCHED"].includes(normalizedStatus)
  ) {
    statusClass = "active";
  } else if (["RESOLVED", "CLOSED", "COMPLETED"].includes(normalizedStatus)) {
    statusClass = "resolved";
  } else if (["CANCELLED", "REJECTED", "FAILED"].includes(normalizedStatus)) {
    statusClass = "cancelled";
  }

  return L.divIcon({
    className: `command-center-marker command-center-marker-${statusClass}`,

    html: `
      <div class="cc-map-marker">
        <span class="cc-map-marker-icon">${emoji}</span>
      </div>
    `,
...
async function startCommandCenterGISWhenReady() {
  if (window.commandCenterAuthReady) {
    const user = await window.commandCenterAuthReady;
  ...
  /*
   * Wait until location is selected.
   */

  if (!CommandCenter.location.selected) {
    showCommandLocationGate();
...
async function loadCompleteCommandCenterData() {
  const location = getCommandLocationState();
  ...
  if (
    ...
    console.log("[COMMAND CENTER] Waiting for location selection.");

    return [];
  }

  console.log("[DASHBOARD] Loading operational data for:", location.name);
...
async function initializeCommandCenterDashboard() {
  ...
  const user = await waitForCommandCenterAuth();

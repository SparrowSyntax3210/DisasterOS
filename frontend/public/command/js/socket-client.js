"use strict";

console.log("🔌 Socket Client Loaded");

let commandSocket = null;

/* ==========================================================
   CONNECT
========================================================== */

function initializeCommandSocket() {
  if (typeof io !== "function") {
    console.error("❌ Socket.IO library not loaded");

    return null;
  }

  if (commandSocket && commandSocket.connected) {
    return commandSocket;
  }

  const socketUrl =
    window.COMMAND_CONFIG?.SOCKET_URL ||
    window.COMMAND_SOCKET_URL ||
    "http://localhost:4000";

  console.log("🔌 Connecting Command Socket:", socketUrl);

  commandSocket = io(socketUrl, {
    transports: ["websocket", "polling"],

    withCredentials: true,
  });

  /* ========================================================
     CONNECTED
  ======================================================== */

  commandSocket.on("connect", () => {
    console.log("🟢 Command Socket Connected:", commandSocket.id);

    window.commandSocket = commandSocket;
  });

  /* ========================================================
     DISCONNECTED
  ======================================================== */

  commandSocket.on("disconnect", (reason) => {
    console.warn("🔴 Command Socket Disconnected:", reason);
  });

  /* ========================================================
     ERROR
  ======================================================== */

  commandSocket.on("connect_error", (error) => {
    console.error("❌ Socket connection error:", error.message);
  });

  /* ========================================================
     GLOBAL
  ======================================================== */

  window.commandSocket = commandSocket;

  return commandSocket;
}

/* ==========================================================
   EXPORT
========================================================== */

window.initializeCommandSocket = initializeCommandSocket;

console.log("✅ Socket Client Ready");

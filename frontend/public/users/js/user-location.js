...
function initializeMap() {
  ...
  map.on("click", function (event) {
    ...
    setLocation(lat, lng, "Selected Map Location");
  });
}
...
function setLocation(lat, lng, name) {
  ...
  marker
    .bindPopup(`<strong>📍 ${escapeHtml(selectedName)}</strong>`)
    .openPopup();
  ...
  selectedCoordinates.textContent = `${selectedLat.toFixed(6)}, ${selectedLng.toFixed(6)}`;
...
currentLocationBtn.addEventListener("click", () => {
  ...
  currentLocationBtn.querySelector("strong").textContent =
    "Detecting location...";
...
async function searchLocation() {
  ...
  try {
    ...
    setLocation(lat, lng, location.name || query);
